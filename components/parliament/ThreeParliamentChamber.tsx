...
export function ThreeParliamentChamber({
  ...
  const presetTriggerRef = useRef<((preset: CameraPreset) => void) | null>(
    null,
  );
  const selectSeatIn3DRef = useRef<((seatSlug: string | null) => void) | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/parliament/seats?chamber=${encodeURIComponent(chamber)}`)
      ...
      .catch(() => {
        setError(
          "Unable to load parliamentary seat records. Please try again.",
        );
        setLoading(false);
      });
  ...
  useEffect(() => {
    ...
    const initialPos =
      width < 640 ? new THREE.Vector3(0, 50, 74) : new THREE.Vector3(0, 44, 66);
    ...
    try {
      ...
      setError(
        "WebGL 3D graphics acceleration is not supported or is disabled in your browser. Seating listings are available below.",
      );
      return;
    }
    ...
    const onContextLost = (e: Event) => {
      e.preventDefault();
      setError(
        "WebGL graphics context was lost. Please refresh or browse representatives below.",
      );
    };
    ...
    const daisSlateMat = new THREE.MeshStandardMaterial({
      color: 0x1e293b,
      metalness: 0.2,
      roughness: 0.45,
    });
    ...
    const upperDaisMat = new THREE.MeshStandardMaterial({
      color: 0x0f172a,
      metalness: 0.25,
      roughness: 0.4,
    });
    ...
    const houseTableMat = new THREE.MeshStandardMaterial({
      color: 0x334155,
      metalness: 0.15,
      roughness: 0.5,
    });
    ...
    const wellRingMat = new THREE.MeshStandardMaterial({
      color: 0xcbd5e1,
      roughness: 0.6,
    });
    ...
    deskFrontGeo.translate(0, 0.19, 0.6);
    ...
    const othersList = seats.filter(
      (s) => s.alliance !== "NDA" && s.alliance !== "INDIA",
    );
    ...
    for (let r = 0; r < numRows; r++) {
      ...
      for (let i = 0; i < countNDA; i++) {
        ...
        const angle =
          startAngle + (i + 0.5) * (ndaSpan / Math.max(countNDA, 1));
      ...
      for (let i = 0; i < countOthers; i++) {
        ...
        const angle =
          othersStartAngle +
          (i + 0.5) * (othersSpan / Math.max(countOthers, 1));
        createSeatMesh(
          seatData,
          angle,
          radius,
      ...
      for (let i = 0; i < countINDIA; i++) {
        ...
        const angle =
          indiaStartAngle + (i + 0.5) * (indiaSpan / Math.max(countINDIA, 1));
        createSeatMesh(
          seatData,
          angle,
          radius,
    ...
    function createSeatMesh(
      ...
      const direction = PRESIDING_CHAIR.clone().sub(
        new THREE.Vector3(x, yElevation, z),
      );
      direction.y = 0;
      direction.normalize();
    ...
    selectSeatIn3DRef.current = (slug: string | null) => {
      ...
      if (match) {
        if (currentSelectedMesh && currentSelectedMesh !== match) {
          ...
          if (prevGroup)
            prevGroup.position.y = currentSelectedMesh.userData.originalY;
        }
        ...
        if (group) {
          ...
          selectionRing.position.set(
            group.position.x,
            match.userData.originalY + 0.05,
            group.position.z,
          );
          selectionRing.visible = true;

          selectionBeacon.position.set(
            group.position.x,
            match.userData.originalY,
            group.position.z,
          );
      ...
      if (event.pointerType === "mouse" && !isDraggingGesture) {
        ...
        if (intersects.length > 0) {
          ...
          if (currentHoveredMesh !== hitMesh) {
            if (
              currentHoveredMesh &&
              currentHoveredMesh !== currentSelectedMesh
            ) {
  ...
  return (
    <div
      className={cn(
        "relative w-full rounded-2xl bg-card border border-border shadow-sm overflow-hidden",
        className,
      )}
    >
      ...
      <div className="p-4 sm:p-6 border-b border-border/70 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-muted/20">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">
            3D seat distribution visualization • Drag to rotate • Pinch/scroll
            to zoom • Tap seat for details
          </p>
        </div>
        ...
        <div
          ...
          role="toolbar"
          aria-label="Alliance seating filters"
        >
          <button
      ...
      <div className="relative w-full h-[400px] sm:h-[480px] lg:h-[540px] cursor-grab active:cursor-grabbing bg-gradient-to-b from-card via-background to-background select-none">
          ...
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-card/85 backdrop-blur-xs overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center text-center p-6 rounded-2xl bg-card/90 border border-border/80 shadow-xs max-w-sm">
              ...
              <p className="text-sm font-semibold text-foreground">
                Constructing 3D {chamber} Chamber...
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Mapping parliamentary seating tiers and alliances
              </p>
            </div>
          </div>
        )}
        ...
        <div
          className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10 flex items-center p-1 rounded-xl bg-background/90 backdrop-blur-md border border-border shadow-md"
          role="toolbar"
          ...
          <div className="hidden sm:flex absolute top-4 right-4 z-10 max-w-xs p-3 rounded-xl bg-background/95 backdrop-blur-md border border-border shadow-xl pointer-events-none items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-muted-foreground truncate">
                {hoveredSeat.partyName} ({hoveredSeat.alliance}) •{" "}
      ...
      <div className="px-4 py-3 bg-muted/15 border-t border-border/70 flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs text-muted-foreground gap-3">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-muted text-[11px] font-bold text-foreground border border-border">
            Majority Target:{" "}
            {chamber === "Lok Sabha" ? "272 Seats" : "123 Seats"}
          </span>
        </div>
      </div>
    </div>
